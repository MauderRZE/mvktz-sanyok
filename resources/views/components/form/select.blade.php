...
<div wire:key="select-wrapper-{{ $model }}" x-data="{
    ...
    init() {
        ...
        const observer = new MutationObserver(() => this.updateOptions());
        observer.observe(this.$refs.realSelect, { childList: true, subtree: true, characterData: true });
    },
    updateOptions() {
        const opts = [];
        Array.from(this.$refs.realSelect.options).forEach(opt => {
            opts.push({
                value: opt.value,
                label: opt.text
            });
    ...
    get selectedLabel() {
        let opt = this.options.find(o => o.value == this.value || ((o.value === '' || o.value === null) && (this.value === '' || this.value === null)));
    ...
    selectOption(val) {
        ...
        this.open = false;
        this.search = '';
        this.$refs.realSelect.value = val;
        this.$refs.realSelect.dispatchEvent(new Event('change'));
    }
}" class="relative w-full" @click.away="open = false; search = '';">
    ...
    <!-- Dropdown Content -->
    <div x-show="open" x-transition class="absolute z-50 mt-1 w-full bg-surface-900 border border-white/10 rounded-xl p-2 shadow-2xl space-y-2 max-h-60 flex flex-col" style="display: none;">
        <!-- Search Input - Show only if we have more than 5 options -->
        <div x-show="options.length > 5" class="relative shrink-0">
        ...
        <div class="overflow-y-auto max-h-40 flex-1 space-y-0.5">
            <template x-for="opt in filteredOptions" :key="opt.value">
                <button @click="selectOption(opt.value)" type="button" class="w-full text-left px-3 py-1.5 text-xs text-white hover:bg-brand-500 hover:text-white rounded transition-colors flex items-center justify-between" :class="{ 'bg-brand-500/20 text-brand-300': value == opt.value || ((value === '' || value === null) && (opt.value === '' || opt.value === null)) }">
                    <span x-text="opt.label"></span>
                    <svg x-show="value == opt.value || ((value === '' || value === null) && (opt.value === '' || opt.value === null))" class="w-3.5 h-3.5 text-brand-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                </button>
            </template>
