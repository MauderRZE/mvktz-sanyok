...
<x-ui.page-wrapper>
    <x-ui.flash />
<x-ui.toolbar :count="$movements instanceof \Illuminate\Pagination\LengthAwarePaginator ? $movements->total() : count($movements)" label="Всього переміщень" buttonLabel="Перемістити техніку" />
    ...
    <x-ui.card class="p-4 mb-4 space-y-4">
        <div class="flex flex-col lg:flex-row gap-4 items-center">
            <div class="flex-1 w-full flex gap-2">
                <div class="flex-1">
                    <x-form.search wire:model.blur="search" x-on:keydown.enter="$el.blur()" placeholder="Пошук за інвентарним номером, описом, співробітником, кабінетом..." />
                </div>
                @if($search !== '' || !empty($filterAsset) || !empty($filterLocation) || !empty($filterEmployee))
