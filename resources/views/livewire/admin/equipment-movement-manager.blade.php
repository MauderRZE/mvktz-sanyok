<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p class="text-sm text-gray-500">Всього переміщень: <span class="text-gray-300 font-medium">{{ count($movements) }}</span></p>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md bg-surface-800 border border-white/5 rounded-2xl shadow-2xl fade-in overflow-hidden">
            <div class="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-white">{{ $movementId ? 'Редагувати' : 'Зареєструвати' }} переміщення</h3>
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-white/5">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Нове розташування</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Матеріально відповідальний</th>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/[0.02] transition-colors text-sm">
                    <td class="px-5 py-3 text-gray-400 whitespace-nowrap">{{ $m->move_date }}</td>
                    ...
                    <td class="px-5 py-3 text-gray-300">Кабінет {{ $m->location->room_number ?? '-' }}</td>
                    <td class="px-5 py-3 text-gray-300">{{ $m->employee->fullName ?? 'На складі (без відповідального)' }}</td>
                ...
                @empty
                <tr><td colspan="5" class="px-5 py-10 text-center text-gray-600">Немає записів</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
