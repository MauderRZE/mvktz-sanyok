...
<x-ui.page-wrapper>
    ...
    <x-ui.card class="p-4 mb-4 space-y-4">
        <div class="flex flex-col lg:flex-row gap-4 items-center">
            <div class="flex-1 w-full flex gap-2">
                <div class="flex-1">
                    <x-form.search wire:model.live.debounce.300ms="search" placeholder="Пошук за інвентарним номером, назвою характеристики, значенням..." />
                </div>
    ...
    <x-table.wrapper>
        <x-slot name="headers">
            <x-table.th align="left" width="20" wire:click="sortBy('id')" class="cursor-pointer hover:bg-white/5">
            ...
            <x-table.th align="left" wire:click="sortBy('inv_number')" class="cursor-pointer hover:bg-white/5">
                <div class="flex items-center gap-1">Прив'язка / Інв. № @if($sortField === 'inv_number') <span class="text-brand-400">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif</div>
            </x-table.th>
