...
<x-ui.page-wrapper>
    <x-ui.flash />
    <x-ui.toolbar :count="$phones->total()" label="Всього телефонів" buttonLabel="Додати телефон" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full lg:w-auto">
                <x-form.multi-select label="Тип телефону" :selectedCount="count($filterPhoneType)">
                    <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                        <input type="checkbox" value="null" wire:model.live="filterPhoneType" class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
