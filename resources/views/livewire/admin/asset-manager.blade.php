<div>
    <x-ui.page-wrapper>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <p class="text-sm text-gray-500">Всього записів: <span
                        class="text-gray-300 font-medium">{{ $assets->total() }}</span></p>
            </div>
            ...
            getReportUrl() {
                ...
                <button @click="showPrintModal = true"
                    class="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium transition-colors rounded-xl bg-surface-800 text-white hover:bg-surface-700 border border-white/10">
                    🖨 Друк звіту
                </button>
                ...
                <template x-teleport="body">
                    <div x-show="showPrintModal"
                        ...
                        <div @click.away="showPrintModal = false"
                            class="relative w-full max-w-sm bg-surface-800 border border-white/5 rounded-2xl shadow-2xl p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex gap-4">
                                    <label class="flex items-center gap-2 text-sm text-gray-300">
                                        <input type="radio" x-model="orientation" value="landscape"
                                            class="text-brand-500 bg-surface-900 border-white/10"> Альбомна
                                    </label>
                                    <label class="flex items-center gap-2 text-sm text-gray-300">
                                        <input type="radio" x-model="orientation" value="portrait"
                                            class="text-brand-500 bg-surface-900 border-white/10"> Книжкова
                                    </label>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    <template
                                        x-for="(label, key) in {'num':'№ з/п', 'id':'ID', 'equipment':'Пристрій (Інв. №)', 'component_type':'Тип компонента', 'model_sn':'Модель / S/N', 'year':'Рік закупівлі', 'specs':'Специфікації', 'network':'Мережа', 'location':'Місце / Власник', 'status':'Статус', 'notes':'Примітки'}">
                                        <label class="flex items-center gap-2 text-sm text-gray-300">
                                            <input type="checkbox" x-model="columns" :value="key"
                                                class="rounded text-brand-500 bg-surface-900 border-white/10">
                            ...
                            <div class="pt-4 flex justify-end gap-2">
                                <button @click="showPrintModal = false"
                                    class="px-4 py-2 text-sm text-gray-400 hover:text-white">Скасувати</button>
                                <a :href="getReportUrl()" target="_blank" @click="showPrintModal = false"
                                    class="px-4 py-2 text-sm font-medium text-white bg-brand-600 rounded-xl hover:bg-brand-500">Сформувати</a>
                            </div>
                        </div>
                    </div>
                </template>
        ...
        <x-ui.card class="p-4 space-y-4">
            <div class="flex flex-col gap-4">
                <div class="w-full flex gap-2">
                    <div class="flex-1">
                        <x-form.search wire:model.blur="search" x-on:keydown.enter="$el.blur()"
                            placeholder="Пошук по всіх полях (серійний номер, IP, модель, кабінет, ПІБ, обладнання...)" />
                    </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 w-full">
                    <x-form.multi-select label="Категорія" :selectedCount="count($filterCategory)">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterCategory"
                        ...
                        <label
                            ...
                            <input type="checkbox" value="{{ $cat->id }}" wire:model.live="filterCategory"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                    ...
                    <x-form.multi-select label="Тип компонента" :selectedCount="count($filterBaseComponent)">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterBaseComponent"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>[Не вказано / Null]</span>
                        </label>
                        ...
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="{{ $bc->id }}" wire:model.live="filterBaseComponent"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                    ...
                    <x-form.multi-select label="Бренд" :selectedCount="count($filterBrand)">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterBrand"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>[Не вказано / Null]</span>
                        </label>
                        ...
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="{{ $brand->id }}" wire:model.live="filterBrand"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                    ...
                    <x-form.multi-select label="Моделі" :selectedCount="count($filterModel)">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterModel"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>[Не вказано / Null]</span>
                        </label>
                        ...
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="{{ $m->id }}" wire:model.live="filterModel"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                    ...
                    <x-form.multi-select label="Локації" :selectedCount="count($filterLocation)">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterLocation"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>[Не вказано / Null]</span>
                        </label>
                        ...
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="{{ $loc->id }}" wire:model.live="filterLocation"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                    ...
                    <x-form.multi-select label="Власники" :selectedCount="count($filterHolder)">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterHolder"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>[Не вказано / Null]</span>
                        </label>
                        ...
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="{{ $h->id }}" wire:model.live="filterHolder"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span class="truncate"
                                title="{{ $h->employee ? $h->employee->last_name . ' ' . mb_substr($h->employee->first_name, 0, 1) . '.' . ($h->organization ? ' (' . $h->organization->org_name . ')' : '') : ($h->organization->org_name ?? 'Невідомий') }}">
                    ...
                    <x-form.multi-select label="Мережа" :selectedCount="count($filterNetwork)">
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="1" wire:model.live="filterNetwork"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>Є мережа</span>
                        </label>
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="0" wire:model.live="filterNetwork"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                    ...
                    <x-form.multi-select label="Статуси" :selectedCount="count($filterStatus)">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterStatus"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>[Не вказано / Null]</span>
                        </label>
                        ...
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="{{ $st }}" wire:model.live="filterStatus"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>{{ $st }}</span>
                        </label>
                        @endforeach
                    ...
                    <x-form.multi-select label="Рік придбання" :selectedCount="count($filterYears ?? [])">
                        <label class="flex items-center gap-2 text-xs font-semibold text-amber-400 cursor-pointer py-1">
                            <input type="checkbox" value="null" wire:model.live="filterYears"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>[Не вказано / Null]</span>
                        </label>
                        {{-- Виводимо список років з PHP-змінної $availableYears (або $years) --}}
                        @foreach($years as $year)
                        <label
                            class="flex items-center gap-2 text-xs text-gray-300 hover:text-white cursor-pointer py-1">
                            <input type="checkbox" value="{{ $year }}" wire:model.live="filterYears"
                                class="rounded border-white/10 bg-surface-900 text-brand-500 focus:ring-0 focus:ring-offset-0">
                            <span>{{ $year }}</span>
                        </label>
                        @endforeach
        ...
        <div wire:key="asset-modal-wrapper">
            <x-ui.modal wire:key="asset-modal" title="{{ $form->assetId ? 'Редагувати' : 'Додати' }} актив"
                ...
                $pa->id => '#' . $pa->id . ' - ' . ($pa->componentType->component_name ?? '') .
                ($pa->equipment?->inv_number ? '
                [Інв. №' . $pa->equipment->inv_number . ']' : '') . ' (' . ($pa->serial_number ?: 'без S/N') . ')'
                ])->toArray();
                $locOptions = $locationsList->mapWithKeys(fn($loc) => [$loc->id => 'Каб. ' .
                $loc->room_number])->toArray();
                $nomOptions = $nomenclaturesList->mapWithKeys(fn($nom) => [$nom->id => $nom->material_account_name . '
                (' .
                $nom->nomenklature_number . ')'])->toArray();
                $actOptions = $writeOffActsList->mapWithKeys(fn($act) => [$act->id => 'Акт №' .
                $act->act_number])->toArray();

                $holdersOptions = $holdersList->mapWithKeys(function($holder) {
                $empName = $holder->employee ? $holder->employee->last_name . ' ' .
                mb_substr($holder->employee->first_name, 0,
                1) . '.' : null;
                ...
                <!-- тут -->
                <div class="space-y-4">
                    <!-- Перемикач: Створити нове чи Обрати існуюче -->
                    <div class="mb-2">
                        <x-form.checkbox label="Створити нове обладнання під цей актив"
                            model="form.create_new_equipment" :live="true" />
                    </div>

                    <!-- Обладнання та Базовий компонент -->
                    <!-- Блок для створення нового (на всю ширину) -->
                    @if($form->create_new_equipment)
                    <div class="p-3 mb-4 bg-white/5 rounded-lg border border-sky-500/20">
                        <span class="text-xs font-semibold text-sky-400 uppercase tracking-wider block mb-3">Дані нового
                            обладнання</span>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <x-form.input label="Інвентарний номер *" model="form.new_inv_number" type="text"
                                placeholder="напр. 10140001" />
                            <x-form.input label="Найменування" model="form.new_account_name" type="text"
                                placeholder="напр. Комп'ютер персональний" />
                            <x-form.input label="Ціна купівлі" model="form.new_buy_price" type="number" step="0.01"
                                placeholder="0.00" />
                        </div>
                    </div>
                    @endif

                    <!-- Рядок з вибором існуючого та базового компонента -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end mb-4">
                        <div>
                            @if(!$form->create_new_equipment)
                            <x-form.select label="Обладнання (ПК / Пристрій) *" model="form.equipment_id"
                                placeholder="Оберіть обладнання..." :options="$eqOptions" />
                            @endif
                        </div>
                        <div>
                            <x-form.select label="Базовий компонент" model="form.base_component_id"
                                placeholder="Оберіть компонент..." :live="true" :nullable="true"
                                :options="$bcOptions" />
                        </div>
                    </div>

                    <!-- Модель та Батьківський актив -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                        <div>
                            <x-form.select label="Модель" model="form.model_id" placeholder="Оберіть модель..."
                                :options="$modOptions" :live="true" :nullable="true" />
                        </div>
                        <div>
                            @if($isSystemUnit)
                            <div class="opacity-50 pointer-events-none"
                                title="Системний блок не може мати батьківського активу">
                                <x-form.select label="Батьківський актив" model="form.parent_asset_id"
                                    placeholder="Не застосовується" :options="[]" />
                            </div>
                            @else
                            <x-form.select label="Батьківський актив" model="form.parent_asset_id"
                                placeholder="Немає (Основний актив)" :options="$paOptions" :nullable="true" />
                            @endif
                        </div>
                    </div>

                    <!-- Серійний номер та Рік купівлі (2 колонки) -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                        <div>
                            <x-form.input label="Серійний номер" model="form.serial_number" type="text" />
                        </div>
                        <div>
                            <x-form.input label="Рік купівлі" model="form.purchase_year" type="number"
                                placeholder="напр. 2026" min="1900" max="2100" step="1" />
                        </div>
                    </div>

                    <!-- Додаткові примітки та Стан роботи (2 колонки) -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                        <div>
                            <x-form.input label="Додаткові примітки" model="form.notes" type="text"
                                placeholder="напр. розширено пам'ять" />
                        </div>
                        <div>
                    ...
                    <!-- Локація та Відповідальний -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                        <div>
                            <x-form.select label="Локація (Кабінет)" model="form.current_loc_id"
                                placeholder="Не задано..." :options="$locOptions" :nullable="true" />
                        </div>
                        <div>
                            <x-form.select label="Відповідальний" model="form.current_holder_id"
                                placeholder="Не задано..." :options="$holdersOptions" :nullable="true" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
                        <div>
                            <x-form.select label="МШП (Малоцінка)" model="form.nomenclature_id"
                                placeholder="Не прив'язано..." :options="$nomOptions" :nullable="true" />
                        </div>
                        <div>
                            <x-form.select label="Акт списання" model="form.write_off_act_id"
                                placeholder="Не списано..." :options="$actOptions" :nullable="true" />
                        </div>
                    </div>
                    ...
                    <div class="border-t border-white/5 pt-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end mt-4 fade-in">
                            ...
                            <div>
                                <x-form.input label="IP-Адреса" model="form.ip_address" type="text"
                                    placeholder="192.168.1.50" />
                            </div>
                            <div>
                                <x-form.input label="MAC-Адреса" model="form.mac_address" type="text"
        ...
        <x-table.wrapper>
            <x-slot name="headers">
                ...
                <x-table.th align="left" wire:click="sortBy('equipment')" class="cursor-pointer hover:bg-white/5">
                    <div class="flex items-center gap-1">Пристрій (Інв. №) @if($sortField === 'equipment') <span
                            class="text-brand-400">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif</div>
                </x-table.th>
                <x-table.th align="left" wire:click="sortBy('component_type')" class="cursor-pointer hover:bg-white/5">
                    <div class="flex items-center gap-1">Тип компонента @if($sortField === 'component_type') <span
                            class="text-brand-400">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif</div>
                </x-table.th>
                ...
                <x-table.th align="left" wire:click="sortBy('ip_address')" class="cursor-pointer hover:bg-white/5">
                    <div class="flex items-center gap-1">Мережа @if($sortField === 'ip_address') <span
                            class="text-brand-400">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif</div>
                </x-table.th>
                <x-table.th align="left" wire:click="sortBy('location')" class="cursor-pointer hover:bg-white/5">
                    <div class="flex items-center gap-1">Локація @if($sortField === 'location') <span
                            class="text-brand-400">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif</div>
                </x-table.th>
                <x-table.th align="left" wire:click="sortBy('holder')" class="cursor-pointer hover:bg-white/5">
                    <div class="flex items-center gap-1">Відповідальний @if($sortField === 'holder') <span
                            class="text-brand-400">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif</div>
                </x-table.th>
                <x-table.th align="left" wire:click="sortBy('status')" class="cursor-pointer hover:bg-white/5">
                    <div class="flex items-center gap-1">Статус @if($sortField === 'status') <span
                            class="text-brand-400">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span> @endif</div>
                </x-table.th>
            ...
            <x-table.tr class="{{ in_array($c->id, $expandedRows) ? 'bg-surface-900/50' : '' }}">
                <x-table.td align="center">
                    <button wire:click="toggleRow({{ $c->id }})"
                        class="w-6 h-6 flex items-center justify-center rounded hover:bg-white/10 transition-colors text-gray-400 hover:text-white relative">
                        <svg class="w-4 h-4 transition-transform duration-200 {{ in_array($c->id, $expandedRows) ? 'rotate-90' : '' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                        @if($c->childAssets && $c->childAssets->count() > 0)
                        <span
                            class="absolute -top-1 -right-2 bg-brand-500/20 text-brand-300 px-1 py-0.5 rounded text-[8px] font-bold">{{ $c->childAssets->count() }}</span>
                        @endif
                    </button>
                </x-table.td>
                ...
                <x-table.td align="left">
                    @if($c->model)
                    <div class="font-medium">{{ $c->model->brand->brandtz_name ?? '' }} {{ $c->model->model_name }}
                    </div>
                    @else
                    -
                    @endif
                ...
                <x-table.td align="left">
                    @if($c->holder)
                    @php
                    $empName = $c->holder->employee ? $c->holder->employee->last_name . ' ' .
                    mb_substr($c->holder->employee->first_name, 0, 1) . '.' : null;
                ...
                <x-table.td align="right">
                    <x-ui.action-buttons id="{{ $c->id }}" :viewAction="true"
                        moveAction="$dispatch('openMoveAsset', { id: {{ $c->id }} })" />
                </x-table.td>
            </x-table.tr>
            ...
            <tr class="bg-surface-900/30 border-b border-white/5">
                <td colspan="9" class="px-4 py-3">
                    @if($c->childAssets && $c->childAssets->count() > 0)
                    <div class="mb-4">
                        <span class="text-gray-500 block mb-2 text-xs uppercase tracking-wider">📦 Вкладені компоненти
                            ({{ $c->childAssets->count() }})</span>
                            ...
                            <table class="w-full text-xs text-left">
                                <tbody class="divide-y divide-white/5">
                                    ...
                                    <tr class="hover:bg-white/5 transition-colors">
                                        ...
                                        <td class="py-2 px-3">
                                            <div class="text-white">{{ $child->model->brand->brandtz_name ?? '' }}
                                                {{ $child->model->model_name ?? '' }}
                                            </div>
                                            @if($child->serial_number)
                                            <div class="text-gray-500 font-mono text-[10px]">SN:
                                                {{ $child->serial_number }}</div>
                                            @endif
                                        </td>
                                        <td class="py-2 px-3 text-gray-400">
                                            ...
                                            @if($child->holder)
                                            @php
                                            $empName = $child->holder->employee ? $child->holder->employee->last_name .
                                            ' ' .
                                            mb_substr($child->holder->employee->first_name, 0, 1) . '.' : null;
                                        ...
                                        <td class="py-2 px-3 text-right">
                                            <button wire:click="edit({{ $child->id }})"
                                                class="p-1 text-gray-400 hover:text-brand-400 transition-colors"
                    ...
                    <div class="mb-4">
                        <span class="text-gray-500 block mb-2 text-xs uppercase tracking-wider">🏷 Динамічні властивості
                            ({{ $c->itemProperties->count() }})</span>
                        <div class="flex flex-wrap gap-2">
                            @foreach($c->itemProperties as $prop)
                            <span
                                class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-surface-950 border border-white/5 text-xs transition-colors hover:border-white/10">
        ...
        <x-table.mobile-list>
            ...
            <x-table.mobile-card layout="y-2">
                <x-table.mobile-card-header align="start">
                    <div>
                        <span
                            class="text-xs text-brand-400 font-semibold uppercase tracking-wider block mb-1">{{ $c->componentType->component_name ?? '-' }}</span>
                        <x-ui.text-block
                ...
                <x-table.mobile-card-footer class="grid grid-cols-2 gap-2">
                    ...
                    <div>
                        ...
                        <div class="text-xs">
                            @if($c->holder)
                            @php
                            $empName = $c->holder->employee ? mb_substr($c->holder->employee->first_name, 0, 1) . '. ' .
                            $c->holder->employee->last_name : null;
                            $orgName = $c->holder->organization->org_name ?? null;
                            @endphp
            ...
            <div class="space-y-6">
                <div>
                    <h3 class="text-xl font-bold text-white mb-1">
                        {{ $viewAsset->componentType->component_name ?? 'Актив' }}</h3>
                    <p class="text-brand-400">{{ $viewAsset->model->brand->brandtz_name ?? '' }}
                        {{ $viewAsset->model->model_name ?? '' }}
                    </p>
                    <div class="flex flex-wrap items-center gap-2 mt-3">
                        <x-ui.badge status="{{ $viewAsset->status }}" />
                        @if($viewAsset->serial_number)
                        <span
                            class="px-2 py-1 rounded-md bg-white/5 border border-white/10 text-xs font-mono text-gray-300">SN:
                            {{ $viewAsset->serial_number }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 text-sm">
                    ...
                    <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
                        <span class="text-gray-500 text-xs uppercase tracking-wider block mb-1">🖥 Зв'язок з
                            Обладнанням</span>
                        <div class="text-gray-300">{{ $viewAsset->equipment->account_name }}</div>
                        <div class="text-gray-500 font-mono text-xs mt-0.5">Інв. №:
                            {{ $viewAsset->equipment->inv_number ?? '-' }}
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
                        <span class="text-gray-500 text-xs uppercase tracking-wider block mb-1">📍 Розміщення та
                            Відповідальний</span>
                        ...
                        <div class="text-gray-300">
                            @php
                            $empName = $viewAsset->holder->employee ? $viewAsset->holder->employee->last_name . ' ' .
                            mb_substr($viewAsset->holder->employee->first_name, 0, 1) . '.' : null;
                    ...
                    @if(!empty($viewAsset->ip_address) || !empty($viewAsset->mac_address) ||
                    !empty($viewAsset->hostname))
                    <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
                        <span class="text-gray-500 text-xs uppercase tracking-wider block mb-1">🌐 Мережеві
                            налаштування</span>
                    ...
                    <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
                        ...
                        <div class="text-gray-500 font-mono text-xs mt-0.5">
                            {{ $viewAsset->lowValueMaterial->nomenklature_number }}
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
                        <span class="text-gray-500 text-xs uppercase tracking-wider block mb-1">🔗 Батьківський
                            актив</span>
                        <div class="text-brand-400">#{{ $viewAsset->parentAsset->id }} —
                            {{ $viewAsset->parentAsset->componentType->component_name ?? 'Актив' }}
                        </div>
                        @if($viewAsset->parentAsset->serial_number)
                        <div class="text-gray-500 font-mono text-xs mt-0.5">SN:
                            {{ $viewAsset->parentAsset->serial_number }}</div>
                        @endif
                    </div>
                    @endif
                    ...
                    <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
                        <span class="text-gray-500 text-xs uppercase tracking-wider block mb-2">🏷 Динамічні
                            властивості</span>
                        <div class="space-y-2">
                            @foreach($viewAsset->itemProperties as $prop)
                            <div
                                class="flex items-center justify-between gap-4 py-1.5 {{ !$loop->last ? 'border-b border-white/5' : '' }}">
                                <span class="text-gray-500 text-xs">{{ $prop->attribute->name ?? '—' }}</span>
                                <span
                                    class="text-gray-300 text-sm font-medium text-right">{{ $prop->attr_value }}</span>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="bg-surface-950 p-3 rounded-xl border border-white/5 space-y-2">
                        ...
                        <div class="text-sm font-medium">
                            ...
                            @else
                            <span class="text-gray-300">Договір №
                                {{ $viewAsset->equipment->purchase->contract_number }}</span>
                            @endif

                            @if($viewAsset->equipment->purchase->contract_date)
                            <span class="text-gray-500 text-xs ml-1">
                                від
                                {{ \Carbon\Carbon::parse($viewAsset->equipment->purchase->contract_date)->format('d.m.Y') }}
                            </span>
                            @endif
                        </div>
                    ...
                    <div
                        class="bg-surface-950 p-3 rounded-xl border border-white/5 flex items-center justify-between gap-2">
