  ...
  <x-ui.page-wrapper>
    ...
    <x-ui.card class="p-4 space-y-4">
      <div class="flex flex-col gap-4">
        <div class="w-full flex gap-2">
          ...
          @if($search !== '' || !empty($filterStatus) || !empty($filterCategory) || !empty($filterBaseComponent) ||
    ...
    <div wire:key="asset-modal-wrapper">
      <x-ui.modal wire:key="asset-modal" title="{{ $form->assetId ? 'Редагувати' : 'Додати' }} актив" maxWidth="2xl">
        @php
        $eqOptions = $equipmentList->mapWithKeys(fn($eq) => [$eq->id => $eq->inv_number . ' - ' .
        $eq->account_name])->toArray();
        ...
        $pa->id => '#' . $pa->id . ' - ' . ($pa->componentType->component_name ?? '') . ($pa->equipment?->inv_number ? '
        ...
        $empName = $holder->employee ? $holder->employee->last_name . ' ' . mb_substr($holder->employee->first_name, 0,
        1) . '.' : null;
        ...
        <div class="space-y-4">
          ...
          <div class="mb-2">
            <x-form.checkbox label="Створити нове обладнання під цей актив" model="form.create_new_equipment"
              :live="true" />
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end mb-4">
            ...
            <div>
              <x-form.select label="Базовий компонент" model="form.base_component_id" placeholder="Оберіть компонент..."
                :live="true" :nullable="true" :options="$bcOptions" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
            <div>
              <x-form.select label="Модель" model="form.model_id" placeholder="Оберіть модель..." :options="$modOptions"
                :live="true" :nullable="true" />
            </div>
            <div>
              @if($isSystemUnit)
              <div class="opacity-50 pointer-events-none" title="Системний блок не може мати батьківського активу">
                <x-form.select label="Батьківський актив" model="form.parent_asset_id" placeholder="Не застосовується"
                  :options="[]" />
              </div>
              @else
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
            ...
            <div>
              <x-form.input label="Рік купівлі" model="form.purchase_year" type="number" placeholder="напр. 2026"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
            <div>
              <x-form.select label="Локація (Кабінет)" model="form.current_loc_id" placeholder="Не задано..."
                :options="$locOptions" :nullable="true" />
            </div>
            <div>
              <x-form.select label="Відповідальний" model="form.current_holder_id" placeholder="Не задано..."
                :options="$holdersOptions" :nullable="true" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
            <div>
              <x-form.select label="МШП (Малоцінка)" model="form.nomenclature_id" placeholder="Не прив'язано..."
                :options="$nomOptions" :nullable="true" />
            </div>
            <div>
              <x-form.select label="Акт списання" model="form.write_off_act_id" placeholder="Не списано..."
                :options="$actOptions" :nullable="true" />
            </div>
          </div>
      ...
      <x-table.tr class="{{ in_array($c->id, $expandedRows) ? 'bg-surface-900/50' : '' }}">
        <x-table.td align="center">
          <button wire:click="toggleRow({{ $c->id }})"
            ...
            <svg
              class="w-4 h-4 transition-transform duration-200 {{ in_array($c->id, $expandedRows) ? 'rotate-90' : '' }}"
                  ...
                  <tr class="hover:bg-white/5 transition-colors">
                    ...
                    <td class="py-2 px-3 text-right">
                      <button wire:click="edit({{ $child->id }})"
                        class="p-1 text-gray-400 hover:text-brand-400 transition-colors" title="Редагувати компонент">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="space-y-6">
        <div>
          ...
          <div class="flex flex-wrap items-center gap-2 mt-3">
            ...
            <span class="px-2 py-1 rounded-md bg-white/5 border border-white/10 text-xs font-mono text-gray-300">SN:
        ...
        <div class="grid grid-cols-1 gap-4 text-sm">
          ...
          <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
            ...
            <div class="text-gray-500 font-mono text-xs mt-0.5">Інв. №: {{ $viewAsset->equipment->inv_number ?? '-' }}
            </div>
          </div>
          @endif
          ...
          <div class="bg-surface-950 p-3 rounded-xl border border-white/5">
            ...
            <div class="text-gray-500 font-mono text-xs mt-0.5">{{ $viewAsset->lowValueMaterial->nomenklature_number }}
            </div>
          </div>
          @endif
