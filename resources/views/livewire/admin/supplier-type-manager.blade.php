...
<x-ui.page-wrapper>
    ...
    <x-ui.card class="p-4 mb-4">
        <div class="flex gap-2">
            <div class="flex-1">
                <x-form.search wire:model.live.debounce.300ms="search" placeholder="Пошук за типом постачальника..." />
            </div>
            @if($search !== '')
    ...
    <x-ui.modal title="{{ $typeId ? 'Редагувати' : 'Додати' }} тип" maxWidth="md">
        <x-form.input label="Тип постачальника (ФОП, ТОВ)" model="type_name" type="text" />
    </x-ui.modal>
    @endif
